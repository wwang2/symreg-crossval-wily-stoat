...
## `results.png` — the skim-summary figure (REQUIRED)

`results.png` is the one figure a skim-reader opens before anything else. It is **the** orbit summary artifact. The following are hard rules — figure-critic blocks on any violation:

1. **Multi-panel, not single.** Use `GridSpec` or `plt.subplots(nrows≥2, ncols≥2)`. A single-panel `results.png` is rejected.
2. **Combines qualitative and quantitative.** At least one panel shows quantitative metrics (trajectory, per-strip, ablation bars, distribution) *and* at least one panel shows qualitative content (still frame from `behavior.gif`, schematic overlay, sample visualization). All-quantitative or all-qualitative is a MINOR finding.
3. **Panel labels `(a)`, `(b)`, `(c)` in the top-left of every panel** — `ax.text(-0.12, 1.05, '(a)', transform=ax.transAxes, fontsize=14, fontweight='bold')`.
4. **Skim test.** The metric value and baseline comparison must be readable in ≤10 seconds from the image alone.
5. **Clean layout.** `constrained_layout=True`, shared axes (`sharey=True`/`sharex=True`) where panels compare, no overlapping text or legends covering data. Read the PNG back before committing.

### Canonical 2×2 layout

```
┌──────────────────────────────┬──────────────────────────────┐
│ (a) Metric trajectory /      │ (b) Per-strip / ablation /   │
│     convergence curve        │     distribution comparison  │
├──────────────────────────────┼──────────────────────────────┤
│ (c) Behavior snapshot        │ (d) Marginal / baseline      │
│     (still frame from gif)   │     overlay                  │
└──────────────────────────────┴──────────────────────────────┘
```

Top row quantitative, bottom row qualitative. Adjust rows/cols to the problem (2×3 when you need an extra ablation; 1×3 for simple cases, but ≥2 panels either way).

`behavior.{gif,mp4,webm}` remains the one **consolidated** qualitative animation — separate from `results.png`. It may itself be a multi-panel GIF when several phenomena need animating side-by-side; the rule is "one animation file per orbit, not three scattered clips".
